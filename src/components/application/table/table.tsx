...
const TableCardRoot = ({ children, className, size = "md", ...props }: HTMLAttributes<HTMLDivElement> & { size?: "sm" | "md" }) => {
    return (
        <TableContext.Provider value={{ size }}>
            <div {...props} className={cx("overflow-hidden rounded-[12px] bg-primary ring-1 ring-secondary", className)}>
                {children}
            </div>
    ...
    return (
        <TableContext.Provider value={{ size: context?.size ?? size }}>
            <div className="overflow-x-auto">
                <AriaTable className={(state) => cx("w-max min-w-full", typeof className === "function" ? className(state) : className)} {...props} />
            </div>
        </TableContext.Provider>
    );
};
...
const TableCell = ({ className, children, size: sizeProp, ...props }: TableCellProps) => {
    ...
    return (
        <AriaCell
            ...
            className={(state) =>
                cx(
                    "relative text-sm text-tertiary !align-middle outline-focus-ring focus-visible:z-1 focus-visible:outline-2 focus-visible:-outline-offset-2",
                    size === "sm" && "px-5 py-3",
                    size === "md" && "px-6 py-4",
