...
export function ListingEditForm({
  ...
  return (
    <div className="mx-auto w-full max-w-[1180px] px-6 py-10 sm:px-10 lg:px-14 lg:py-14">
      ...
      <div className="mt-8 rounded-[28px] border border-[#e5e7eb] bg-white p-6 shadow-[0_24px_80px_rgba(23,23,28,0.06)]">
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
          <div>
            ...
            <h1 className="bb-display mt-4 text-[2.2rem] font-medium leading-[1.04] text-[#17171c]">
              Edit {listing.name}
            </h1>
...
function EditFieldControl({
  ...
  if (field.kind === "select") {
    return (
      <label className="grid gap-1.5 text-sm font-medium text-[#212121]">
        <span>{field.label}</span>
        <select
          className="min-h-11 rounded-xl border border-[#d9d9dd] bg-white px-3.5 text-[15px] text-[#17171c] outline-none transition-colors focus:border-[#9b60aa] focus:ring-2 focus:ring-[#9b60aa]/15"
          onChange={(event) => onChange(event.target.value)}
          value={stringValue}
        >
          ...
          {field.options?.map((option) => (
            <button
              className={cn(
                ...
                stringValue === option.value ? "bg-[#17171c] text-white" : "text-[#17171c] hover:bg-[#f7f7f9]",
              )}
              key={option.value}
